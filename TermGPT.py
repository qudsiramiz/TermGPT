...
openai.api_key = (
    open(f"/home/{user_name}/Desktop/git/gpt/open_ai_api_key.txt", "r").read().strip()
)
...
def gpt_query(model="gpt-3.5-turbo-0613", max_retries=5, sleep_time=2):
...
print(
    "The goal for TermGPT is to make rapid prototyping with GPT models even quicker and more fluid."
)
print()
...
while True:
    ...
    print(
        ...
        + 'It looks like there is no current input. Please provide your objectives. \n"'
        + colorama.Fore.YELLOW
        + "--c"
        + colorama.Fore.CYAN
        + '" to clear contextual history. \n"'
        + colorama.Fore.YELLOW
        + "--o"
        + colorama.Fore.CYAN
        + '" to parse previous console command outputs and suggest further commands (can be helpful for big errors)\n"'
        + colorama.Fore.YELLOW
        + "--w"
        + colorama.Fore.CYAN
        + '" to parse a website into context \n"'
        + colorama.Fore.YELLOW
        + "--r [path/to/file.py]"
        + colorama.Fore.CYAN
        + '" to open and read some file into context (can do multiple files, and is embedded into some prompt.) Example: "Please change --r [mplexample.py] to be a dark theme"'
    ...
    if init_input.lower() == "clear" or init_input.lower() == "--c":
        ...
        user_input = (
            "NEW STARTING INPUT: I received the following console outputs after running everything. Is there anything I should do to address any errors, warnings, or notifications?: \n"
            + command_outputs
        )
    ...
    while not GPT_DONE:
        print(
            colorama.Fore.GREEN
            + colorama.Style.DIM
            + "Querying GPT for next command (these are not running yet)..."
            + colorama.Style.RESET_ALL
        )
        reply_content = gpt_query(model="gpt-3.5-turbo-0613")
    ...
    if len(commands) > 0:
        print(
            colorama.Fore.CYAN
            + colorama.Style.BRIGHT
            + "Would you like to run these commands? "
            + colorama.Fore.YELLOW
            + "Read carefully!"
            + colorama.Style.RESET_ALL
            + colorama.Fore.CYAN
            + colorama.Style.BRIGHT
            + " Do not run if you don't understand and want the outcomes. (y/n)"
            + colorama.Style.RESET_ALL
        )
